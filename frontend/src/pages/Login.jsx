  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.data === "Login Successful") {
        // Simple auth for now, normally you'd save a token
        localStorage.setItem("user", form.email);
        navigate("/dashboard");
      } else {
        setMessage({ type: "error", text: res.data });
      }
    } catch (error) {
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-8 relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2 text-sm">
            Sign in to access your phishing detection tools.
          </p>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-slate-400">
          Don't have an account? <Link to="/register" className="text-indigo-400 hover:text-indigo-300 hover:underline transition-colors">Create one</Link>
        </p>
      </div>
    </div>
  );
